...
abstract class ClassroomsRemoteDataSource {
  ...
  Future<ClassroomModel> assignSubject(int classroomId, int subjectId);
}
...
class ClassroomsRemoteDataSourceImpl implements ClassroomsRemoteDataSource {
  ...
  Future<ClassroomModel> getClassroom(int id) {
    return client
        .get(Uri.parse(
            'https://hamon-interviewapi.herokuapp.com/classrooms/$id?api_key=6c5Fc'))
        .then((response) {
  ...
  Future<List<ClassroomModel>> getClassrooms() {
    return client
        .get(Uri.parse(
            'https://hamon-interviewapi.herokuapp.com/classrooms?api_key=6c5Fc'))
        .then((response) {
      if (response.statusCode == 200) {
        final jsonResponse = json.decode(response.body);
        return (jsonResponse['classrooms'] as List)
  ...
  Future<ClassroomModel> assignSubject(int classroomId, int subjectId) {
    ...
    return client
        .patch(
      Uri.parse(
          'https://hamon-interviewapi.herokuapp.com/classrooms/$classroomId?api_key=6c5Fc'),
      body: requestBody,
    )
        .then((response) {
      if (response.statusCode == 200) {
